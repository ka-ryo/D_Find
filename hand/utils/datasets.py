...
class LoadStreams:  # multiple IP or RTSP cameras
    ...
    video = cv2.VideoWriter('./video/video_0.mp4', fourcc, 30, (480, 600))
    w = 0
    h = 0
    fps = 0
    ...
    def __init__(self, sources='streams.txt', img_size=640):
        ...
        for i, s in enumerate(sources):
            ...
            self.video = cv2.VideoWriter('./video/video_0.mp4', self.fourcc, self.fps, (self.w, self.h))  # 動画の仕様（ファイル名、fourcc, FPS, サイズ）
    ...
    def __next__(self):
        ...
        #動画保存
        #self.video.write(cv2.UMat(np.array(img)))  
        self.video.write(self.imgs[0])
        if time.time()-self.start > 60:
            print("=========================================")
            print("SAVE VIDEO")
            print("=========================================")
            self.start = time.time()
            self.video = cv2.VideoWriter('video/video_{}.mp4'.format(self.counter), self.fourcc, self.fps, (self.w, self.h))  # 動画の仕様（ファイル名、fourcc, FPS, サイズ）
            self.counter+=1

            if self.counter == 14:
                #一番最後の動画は削除
                if os.path.exists('video/video_14.mp4'):
                    os.remove('video/video_14.mp4')
                self.counter =0
