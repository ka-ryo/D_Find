...
def plot_one_box(x, img, count,time,color=None, label=None, line_thickness=None):
    ...
    #手だけの範囲
    """
    with open("./hand_box_save/"+label.split()[0]+".txt",mode="a") as f:
        f.write("{} {} {} {} {}\n".format(c1[0],c1[1],c2[0],c2[1],time))
    """
    ...
    if y2*1.2<img.shape[0]:
        y2 = int(y2*1.2)
    else:
        y2 = img.shape[0]

    x,y,w,h=x1, y1, x2-x1, y2-y1
    #trimming_img=img[trimmingx1:trimmingx2,trimmingy1:trimmingy2]
    trimming_img=img[y:y+ h, x:x + w] 

    #拡大した範囲
    #with open("./hand_box_save/"+label.split()[0]+".txt",mode="a") as f:
    with open("./hand_box_save/hand.txt",mode="a") as f:
        f.write("{} {} {} {} {}\n".format(x1,y1,x2,y2,time))
        
    """
    if trimmingx2/1.2 > 0:
        trimmingx2 = int(trimmingx2/1.2)

    if trimmingx1*1.2 < img.shape[0]:
        trimmingx1 = int(trimmingx1*1.2)
    else:
        trimmingx1 = img.shape[0]
    

    if trimmingy1*1.2 < img.shape[1]:
        trimmingy1 = int(trimmingy1*1.2)
    else:
        trimmingy1 = img.shape[1]

    
    if trimmingy2/1.2 > 0:
        trimmingy2 = int(trimmingy2/1.2)

    trimming_img=img[trimmingx2:trimmingx1,trimmingy2:trimmingy1]
    """


    #print(img.shape)
    #print(c1[1],c2[1],c1[0],c2[0])
    #print(trimmingx1,trimmingy1,trimmingx2,trimmingy2)
    #trimming_img=img[trimmingx1:trimmingx2,trimmingy1:trimmingy2]

    os.makedirs('./trimming', exist_ok=True)
    cv2.imwrite("./trimming/{}.jpg".format(count),trimming_img)
