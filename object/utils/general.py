...
def plot_one_box(x, img,count=-1,color=None, label=None, line_thickness=None):
    # Plots one bounding box on image img
    tl = line_thickness or round(0.002 * (img.shape[0] + img.shape[1]) / 2) + 1  # line/font thickness
    color = color or [random.randint(0, 255) for _ in range(3)]
    c1, c2 = (int(x[0]), int(x[1])), (int(x[2]), int(x[3]))
    #バウンディングボックスの座標をメモ
    os.makedirs("./Object_box_save",exist_ok=True)
    if count != -1:
        if label:
            with open("./Object_box_save/"+label.split()[0]+".txt",mode="a") as f:
                f.write("{} {} {} {} {}\n".format(count,c1[0],c1[1],c2[0],c2[1]))

        #trimmnin画像の保存
        x,y,w,h=c1[0], c1[1], c2[0]-c1[0], c2[1]-c1[1]
        
        trimming_img=img[y:y+h,x:x +w] 
        os.makedirs('./trimming_obj', exist_ok=True)
        os.makedirs('./trimming_obj/{}'.format(label.split()[0]), exist_ok=True)
        cv2.imwrite("./trimming_obj/{}/{}.jpg".format(label.split()[0],count),trimming_img)
